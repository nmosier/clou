...
TODO: brief description. Note that there's an implicit timeout?
Options:
  ...
  -T <file-tmout>  per-file analysis timeout, in seconds [optional, default: 3600]
...
SPECTRE_TYPE=
FILE_TIMEOUT=3600
while getopts "ht:p:j:J:T:" OPTC; do
      case $OPTC in
	  h)
	      usage
	      exit
...
export THREADS
. "${LCM_ARGS_FILE}"
export LCM_ARGS="$(get_lcm_args "$SPECTRE_TYPE") --file-timeout=$FILE_TIMEOUT $@"
